...
Public Class ControlRepairCollection5
    ...
    Private Sub grdTechnicianCost_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs)
        ...
        If CheckEmptyControl(ParentRepairForm.ControlTechnicianInfo.ComboHandOverToTechnician, "Technician Name යන field එක හිස්ව පවතියි. කරුණාකර එය සම්පුර්ණ කරන්න.") = False Then
        ...
        If grdTechnicianCost.Item(0, e.RowIndex).Value Is Nothing Then
            ...
            Db.Execute($"INSERT INTO {Tables.TechnicianCost}(TCNo) Values(" & grdTechnicianCost.Item(0, e.RowIndex).Value & ")",
                      {}, AdminPer)
        End If
        If Db.CheckDataExists("TechnicianCost", "TCNo", grdTechnicianCost.Item(0, e.RowIndex).Value) = True Then
            Db.Execute($"UPDATE {Tables.TechnicianCost} SET TCDate='" & grdTechnicianCost.Item("TCDate", e.RowIndex).Value &
                      "',TNo" = Db.GetData($"SELECT TNo from Technician WHERE TName='{ParentRepairForm.ControlTechnicianInfo.ComboHandOverToTechnician.Text}'") &
