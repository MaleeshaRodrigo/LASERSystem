...
Public Class frmReceive
    ...
    Private Sub cmbCuName_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbCuName.SelectedIndexChanged
        If txtCuTelNo1.Text.Trim = "" Then
            Dim DrCheckCustomerExist As MySqlDataReader = Db.GetDataReader("SELECT * from Customer where CuName='" & cmbCuName.Text & "' and CuTelNo1='" & txtCuTelNo1.Text &
            ...
            Else
                For i As Integer = 0 To 1000
                    Dim DrCuNameSuggest As MySqlDataReader = Db.GetDataReader("Select CuName from Customer Where CuName = '" & cmbCuName.Text & " " & i.ToString & "'")
        ...
        Else
            ...
            Dim DrCuName As MySqlDataReader = Db.GetDataReader("Select CuName from Customer where CuName ='" & cmbCuName.Text & "';")
    ...
    Private Sub SaveReceive()
        ...
        Dim DR As MySqlDataReader = Db.GetDataReader("Select * from Customer where CuName='" & cmbCuMr.Text & cmbCuName.Text & "' and CuTelNo1='" & txtCuTelNo1.Text & "' and CuTelNo2 ='" & txtCuTelNo2.Text & "' and CuTelNo3='" & txtCuTelNo3.Text & "'")
        ...
        For Each row As DataGridViewRow In grdRepair.Rows
            If row.Index = grdRepair.Rows.Count - 1 Then Continue For
            'Product Management
            Dim DrProduct As MySqlDataReader = Db.GetDataReader("Select * from Product where PCategory='" & row.Cells(1).Value & "' and PName='" & row.Cells(2).Value & "'")
        ...
        For Each row As DataGridViewRow In grdReRepair.Rows
            If row.Index = grdReRepair.Rows.Count - 1 Then Continue For
            'Product Management
            Dim DrProduct As MySqlDataReader = Db.GetDataReader("Select * from Product where PCategory='" & row.Cells(2).Value & "' and PName = '" & row.Cells(3).Value & "'")
            If DrProduct.HasRows = True Then
                DrProduct.Read()
                PNo = DrProduct("PNo")
            Else
            ...
            Sub()
                ...
                Try
                    ...
                    For Each row As DataRow In DTRepair.Rows
                        Dim DrRemarks As MySqlDataReader = ThreadDb.GetDataReader($"Select Remarks from RepairRemarks1 Where {If(row.Item("RetNo") = "", $"RepNo={row.Item("RepNo")}", $"RetNo={row.Item("RetNo")}")};")
        ...
        Select Case grdRepair.CurrentCell.ColumnIndex
            Case 1
                ...
                If autoText IsNot Nothing Then
                    ...
                    Dim DrProduct As MySqlDataReader = Db.GetDataReader("Select PCategory from Product group by PCategory;")
            ...
            Case 2
                ...
                If autoText IsNot Nothing Then
                    ...
                    autoText.AutoCompleteSource = AutoCompleteSource.CustomSource
                    DataCollection.Clear()
                    Dim DR As MySqlDataReader = Db.GetDataReader("Select PCategory,PName from Product where PCategory ='" & grdRepair.Item(1, grdRepair.CurrentCell.RowIndex).Value & "';")
                    While DR.Read
    ...
    Public Sub grdRepair_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles grdRepair.CellEndEdit
        Select Case e.ColumnIndex
            Case 1, 2
                ...
                End If
                Dim DR As MySqlDataReader = Db.GetDataReader("Select * from Product where PCategory='" & grdRepair.Item(1, e.RowIndex).Value & "' and PName='" & grdRepair.Item(2, e.RowIndex).Value & "';")
                If DR.HasRows = True Then
                    DR.Read()
    ...
    Private Sub grdRepair_UserAddedRow(sender As Object, e As DataGridViewRowEventArgs) Handles grdRepair.UserAddedRow
        Dim i As Integer
        Dim DR As MySqlDataReader = Db.GetDataReader("Select Top 1 REpNo from REpair Order by repno desc;")
        If DR.HasRows = True Then
            DR.Read()
    ...
    Public Sub GrdReRepair_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles grdReRepair.CellEndEdit
        Select Case e.ColumnIndex
            Case 1
                ...
                Dim DrCustomer As MySqlDataReader = Db.GetDataReader("Select RepNo,Rep.RNo,Cu.CuNo,CuName,CuTelNo1,CuTElNo2,CuTelNo3 from Repair REP, Receive R, Customer Cu Where Rep.RNo = R.RNo and Cu.CuNo = R.CuNo and RepNo = " & grdReRepair.Item(1, e.RowIndex).Value)
                ...
                End If
                Dim DrProduct As MySqlDataReader = Db.GetDataReader("Select RepNo,PCategory,PName,PModelNo,PSerialNo,PDetails,Qty from Repair Rep,Product P where Rep.Pno = P.PNo and RepNo=" & grdReRepair.Item(1, e.RowIndex).Value)
                If DrProduct.HasRows = True Then
                    DrProduct.Read()
    ...
    Public Sub grdReRepair_UserAddedRow(sender As Object, e As DataGridViewRowEventArgs) Handles grdReRepair.UserAddedRow
        Dim i As Integer
        Dim DR As MySqlDataReader = Db.GetDataReader("Select Top 1 RetNo from Return Order by retno desc;")
        If DR.HasRows = True Then
            DR.Read()
    ...
    Private Sub txtCuTelNo1_KeyUp(sender As Object, e As KeyEventArgs) Handles txtCuTelNo1.KeyUp
        ...
        Dim SaDR As MySqlDataReader = Db.GetDataReader("Select * from Customer where CuTelNo1='" & txtCuTelNo1.Text & "' or CuTelNo2='" & txtCuTelNo1.Text &
