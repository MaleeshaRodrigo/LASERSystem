Public Class frmStockTransaction
    ...
    Private Sub cmbSName_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbSName.SelectedIndexChanged
        ...
        If DR.HasRows = True Then
            ...
            Dim DT1 As DataTable = Db.GetDataTable("Select SupDate,SuName,SupType,SupUnits,SupCostPrice,SupTotal from (((Supply Sup Left Join Supplier Su On Su.SuNo = Sup.SuNo) " &
            ...
            Dim DT2 As DataTable = Db.GetDataTable("Select SaDate,CuName,SaType,SaUnits,SaRate,SaTotal from (((Sale Sa Left Join Customer Cu On Cu.CuNo = Sa.CuNo) Inner Join StockSale SSa " &
                                                  "On SSa.SaNo = Sa.SaNo) Inner Join Stock S On S.SNo =SSa.SNo) Where S.SNo = " & txtSNo.Text & " Order by SaDate;")
            grdSale.DataSource = DT2

            Dim DT3 As DataTable = Db.GetDataTable("Select TCDate,TName,RepNo,RetNo,TCRemarks,Rate,Qty,Total from ((TechnicianCost TC Inner Join Stock S On S.SNo = TC.SNo) " &
                                                  "Inner Join Technician T On T.TNo = TC.TNo) Where S.SNo = " & txtSNo.Text & " Order by TCDate;")
            grdTCost.DataSource = DT3

            Dim DT4 As DataTable = Db.GetDataTable("Select TLDate,TName,TLReason,Rate,Qty,Total from ((TechnicianLoan TL Inner Join Stock S On S.SNo = TL.SNo) " &
    ...
    Public Sub txtSNo_TextChanged(sender As Object, e As EventArgs) Handles txtSNo.TextChanged
        ...
        DR = Db.GetDataReader("Select SNo,SCategory,SName from [Stock] where SNO = " & txtSNo.Text)
        If DR.HasRows = True Then
            DR.Read()
    ...
    Public Sub cmdDone_Click(sender As Object, e As EventArgs) Handles cmdDone.Click
        If CheckEmptyfield(txtSNo, "ඔබ තවම Stock එකක් Select කර නොමැත. කරුණාකර Stock එකක් තෝරා ගෙන නැවත උත්සහ කරන්න.") = False Then
            Exit Sub
        End If
        Dim DT1 As DataTable = Db.GetDataTable("Select SupDate,SuName,SupType,SupUnits,SupCostPrice,SupTotal from StockSupply SSup,Supplier Su,Supply Sup Where Sup.SupNo = SSup.SupNo and Sup.SuNo = Su.SuNo and SNo = " & txtSNo.Text)
        grdSupply.DataSource = DT1
        Dim DT2 As DataTable = Db.GetDataTable("Select SaDate,CuName,SaType,SaRate,SaUnits,SaTotal from Sale Sa, StockSale SSa, Customer Cu Where Cu.CuNo = Sa.CuNo and Sa.SaNo = SSa.SaNo and SNo = " & txtSNo.Text)
        grdSale.DataSource = DT2
        Dim DT3 As DataTable = Db.GetDataTable("Select TCDate, TName, RepNo, RetNo, TCRemarks, Rate, Qty, Total from TechnicianCost TC, Technician T Where T.TNo = TC.TNo and SNo = " & txtSNo.Text)
        grdTCost.DataSource = DT3
        Dim DT4 As DataTable = Db.GetDataTable("Select TLDate, TName, TLReason, Rate, Qty, Total from TechnicianLoan TL, Technician T Where T.TNo = TL.TNo and SNo = " & txtSNo.Text)
        grdTLoan.DataSource = DT4
    End Sub
