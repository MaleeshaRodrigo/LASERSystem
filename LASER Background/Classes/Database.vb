...
Public Class Database
    ...
    Public Sub Connect()
        ...
        For i As Integer = 0 To 3
            Try
                _Connection = New OleDbConnection($"Provider={Settings.DatabaseProvider};Data Source={Settings.DatabasePath};Jet OLEDB:Database Password={(New Encoder()).Decode(Settings.DatabasePassword)};")
                _Connection.Open()
                Exit For
    ...
    Public Function CheckConnection() As (Valid As Boolean, Message As String)
        If Settings.DatabaseProvider = "" Then
            Return (False, "Database Provider ඇතුලත් කර නොමැත.")
        End If
        If Settings.DatabasePath = "" Then
            Return (False, "Database Path එක ඇතුලත් කර නොමැත.")
        End If
        If Settings.DatabasePassword = "" Then
            Return (False, "Database Password එක ඇතුලත් කර නොමැත.")
        End If
        If File.Exists(Settings.DatabasePath) = False Then
    ...
    Private Function FormatQuery(Query As String) As String
