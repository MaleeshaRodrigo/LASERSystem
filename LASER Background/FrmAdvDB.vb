...
Public Class FrmAdvDB
    ...
    Private Sub FrmAdvDB_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        CNN = New OleDbConnection("Provider=" & My.Settings.DBProvider & ";Data Source=" &
                                            My.Settings.DatabaseCNN & ";Jet OLEDB:Database Password=" &
                                            frmBGTasks.Simple.Decode(My.Settings.DBPassword) & ";")
        CNN.Open()
    End Sub
    ...
    Private Sub CmbOTable_DropDown(sender As Object, e As EventArgs) Handles CmbOTable.DropDown
        ...
        Dim JSONResponse As String = frmBGTasks.GetResponse(My.Settings.ODBPath + "/view", $"user={frmBGTasks.HashPassword(My.Settings.ODBUser, frmBGTasks.Simple.Decode(My.Settings.ODBToken))}&password={frmBGTasks.HashPassword(frmBGTasks.Simple.Decode(My.Settings.ODBPassword) + Format(Now(), "yyyy-MM-dd"), frmBGTasks.Simple.Decode(My.Settings.ODBToken))}&sql=SHOW tables;")
    ...
    Private Sub CmbOTable_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CmbOTable.SelectedIndexChanged
        ...
        Dim JSONResponse As String = frmBGTasks.GetResponse(My.Settings.ODBPath + "/view", $"user={frmBGTasks.HashPassword(My.Settings.ODBUser, frmBGTasks.Simple.Decode(My.Settings.ODBToken))}&password={frmBGTasks.HashPassword(frmBGTasks.Simple.Decode(My.Settings.ODBPassword) + Format(Now(), "yyyy-MM-dd"), frmBGTasks.Simple.Decode(My.Settings.ODBToken))}&sql=SELECT * FROM `{CmbOTable.Text}`;")
        Dim BGDT As DataTable = JsonConvert.DeserializeObject(Of DataTable)(JSONResponse)
